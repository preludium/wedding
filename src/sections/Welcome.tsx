  ...
  return (
    <div className="flex flex-col items-center">
      <div className="bg-white/70 backdrop-blur-sm border border-[#e8ca9b] rounded-lg shadow-md w-20 h-20 md:w-24 md:h-24 flex items-center justify-center">
        <span className="text-2xl md:text-3xl font-medium text-slate-800">{Math.round(value)}</span>
      </div>
      <span className="mt-2 font-alice text-sm md:text-base text-slate-700">{label}</span>
    </div>
  );
}
  ...
  const countDown = (
    <div className="flex w-full justify-center">
      <div className="flex flex-col w-1/4 max-w-[250px] gap-10 text-xl">
        <CountdownItem value={Math.max(0, diff.days)} label={t('common.days')} />
      </div>
      <div className="flex flex-col w-1/4 max-w-[250px] gap-10 text-xl">
        <CountdownItem value={Math.max(0, diff.hours)} label={t('common.hours')} />
      </div>
      <div className="flex flex-col w-1/4 max-w-[250px] gap-10 text-xl">
        <CountdownItem value={Math.max(0, diff.minutes)} label={t('common.minutes')} />
      </div>
      <div className="flex flex-col w-1/4 max-w-[250px] gap-10 text-xl">
        <CountdownItem value={Math.max(0, diff.seconds)} label={t('common.seconds')} />
      </div>
    </div>
  );
      ...
      <section className="flex flex-col bg-[#fcfcfc] py-28 gap-12">
        ...
        <div className="container grid grid-cols-2 items-center -md:grid-cols-1 gap-6">
          ...
          <div className="flex">
            <img
              src={photo}
              className="aspect-[4/3] max-h-[376px] rounded-lg shadow-around-sm"
              alt="Gabriela and Francis engagement portrait"
            />
          </div>
        </div>
      </section>
